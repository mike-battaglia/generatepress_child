...
<div class="vendor_description_background wcmp_vendor_banner_template <?php echo $template_class; ?>">
        ...
        <div class="vendor_description">
            <div class="vendor_img_add">
                ...
                <div class="vendor_address">
                    <p class="wcmp_vendor_name artist_name"><?php echo $vendor->page_title ?></p>
					...
					?>
						<p class="wcmp_vendor_detail artist_info"><i class="wcmp-font ico-location-icon"></i><label><?php echo $location; ?></label></p>
					<?php } 
						$vendor_primary_dis = get_user_meta($vendor_id, 'primary_discipline', true);
						if($vendor_primary_dis!='')
						{
					?>
						<p class="wcmp_vendor_detail artist_info"><?php echo $vendor_primary_dis; ?></p>
                    <?php 
						}
                    if (!empty($email) && $vendor_hide_email != 'Enable') { ?><a href="mailto:<?php echo apply_filters('vendor_shop_page_email', $email, $vendor_id); ?>" class="wcmp_vendor_detail"><i class="wcmp-font ico-mail-icon"></i><?php echo apply_filters('vendor_shop_page_email', $email, $vendor_id); ?></a><?php } ?>
                    <?php
    ...
    <div class="vendor_description">
        <div class="vendor_img_add">
            ...
            <div class="">
                <h1 class="wcmp_vendor_name artist_name"><?php echo esc_html($vendor->page_title) ?></h1>
                <?php do_action('before_wcmp_vendor_information',$vendor_id);?>
                <div class="wcmp_vendor_rating">
                    <?php
                    if (get_wcmp_vendor_settings('is_sellerreview', 'general') == 'Enable') {
				...
				//			$location .= ", ". $state;						
                    	...
                    	if($country!='' && $location!='')
							$location .= ", USA";
					?><p class="wcmp_vendor_detail artist_info"><label class="artist-archive-page-state"><?php echo esc_html($location); ?></label></p><br /><?php } ?>
                <?php 
					$vendor_primary_dis = get_user_meta($vendor_id, 'primary_discipline', true);
						if($vendor_primary_dis!='')
						{
					?>
						<p class="wcmp_vendor_detail artist_info" style="font-size:20px;"><?php echo $vendor_primary_dis; ?></p>
                    <?php 
						}
                 ?>
