...
/* change the url of Add Product (to remove the first screen of add product with search) in left side bar */
$add_url = apply_filters('wcmp_vendor_submit_product', esc_url(wcmp_get_vendor_dashboard_endpoint_url(get_wcmp_vendor_settings('wcmp_add_product_endpoint', 'vendor', 'general', 'edit-product'))));
$nav_items['vendor-products']['submenu']['add-product']['url']=$add_url;
/* unset the shipping from menu*/
if(isset($nav_items['store-settings']['submenu']['vendor-shipping']))
	unset($nav_items['store-settings']['submenu']['vendor-shipping']);
