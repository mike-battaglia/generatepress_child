...
/* changes for limit the vendor for adding products */
global $wp;
global $ecVendorProduct;
$id = $wp->query_vars[get_wcmp_vendor_settings( 'wcmp_edit_product_endpoint', 'vendor', 'general', 'edit-product' )];
$current_vendor_id = get_current_user_id();
$products_count = $ecVendorProduct->ec_get_product_count_of_vendor($current_vendor_id);
$products_limit = 30;
if( $products_count >= $products_limit  && $id=="")
{
	?>
	<div class="col-md-12">
		<div class="woocommerce-error">
			<?php echo sprintf( __("Error. You are not allowed to create more than %s products."), $products_limit) ; ?>
		</div>
	</div>
	<?php
}
            ...
            <div class="right-primary-info"> 
                <div class="form-group-wrapper">
                    <div class="form-group product-description">
                        <label class="control-label col-md-12" for="product_description"><?php esc_attr_e( 'Artwork Description', 'woocommerce' ); ?></label>
                        <div class="col-md-12">
                            <?php
                            $settings = array(
                            ...
                            ?>
                            <textarea name="product_description" class="form-control required" rows="30" style="height: 325px;"><?php echo  htmlspecialchars_decode( $product_object->get_description( 'edit' )); ?></textarea>
                        </div>
                        
                    </div>
                     <div class="form-group product-short-description">
                        <label class="control-label col-md-12 pt-0" for="product_short_description"><?php esc_html_e( 'Material/Medium', 'woocommerce' ); ?></label>
                        <div class="col-md-12">
                            <?php
                            $settings = array(
                            ...
                            ?>  
                            <textarea name="product_excerpt" class="form-control required"><?php echo  htmlspecialchars_decode( $product_object->get_short_description( 'edit' )); ?></textarea>
                        </div>
                    </div>
                </div> 
            </div>
            <div class="left-primary-info">
                <div class="product-gallery-wrapper">
                    <div class="featured-img upload_image"><?php $featured_img = $product_object->get_image_id( 'edit' ) ? $product_object->get_image_id( 'edit' ) : ''; ?>
                        <a href="#" class="upload_image_button tips <?php echo $featured_img ? 'remove' : ''; ?>" <?php echo current_user_can( 'upload_files' ) ? '' : 'data-nocaps="true" '; ?>data-title="<?php esc_attr_e( 'Product image', 'woocommerce' ); ?>" data-button="<?php esc_attr_e( 'Set product image', 'woocommerce' ); ?>" rel="<?php echo esc_attr( $post->ID ); ?>">
                            <div class="upload-placeholder pos-middle">
                                ...
                                <p><?php _e( 'Click to upload Image <br/> JPGE, JPG and PNG Only <br/>8MB Max', 'dc-woocommerce-multi-vendor' );?></p>
                            </div>
                    ...
                    <div id="product_images_container" class="custom-panel">
                        <h3><?php _e( 'Additional Artwork Images', 'dc-woocommerce-multi-vendor' );?></h3>
        ...
        <div class="row">
            <div class="col-md-12">
                <div id="woocommerce-product-data" class="add-product-info-holder">   
                    ...
                    <div class="add-product-info-header row-padding hide_me">
        ...
        <div class="row">
            ...
            <div class="col-md-12">
				<div class="row">
					...
					<?php if ( $product_categories ) : ?>
						<div class="panel panel-default pannel-outer-heading">
							<div class="panel-heading">
								<h3 class="pull-left"><b><?php esc_html_e( 'Main Artwork Category ', 'woocommerce' ); ?></b><span>(One choice only. Scroll for all options)</span></h3>
								<div class="taxo_error"></div>
							</div>
							<div class="panel-body panel-content-padding form-group-wrapper"> 
								<?php
								/* changes to replace checkbox with radio in vendor add/edit product */
								echo str_replace('type="checkbox"','type="radio"',$product_categories);
					...
					foreach($taxArr as $taxname => $taxlbl)
					{
					...
					?>
						<div class="col-md-4 <?php echo $taxname; ?>_sec taxo_sec">
							<div class="panel panel-default pannel-outer-heading">
								<div class="panel-heading">
									<h3 class="pull-left">
										<b><?php echo $taxlbl; ?></b>
										<?php 
											if($taxname == "color")
												echo "<span>(Choose up to three (3) colors)</span>"; 
											else if($taxname == "subject" || $taxname == "style" || $taxname == "orientation" || $taxname == "types" || $taxname == "medium")
												echo "<span>(One choice only. Scroll for all options)</span>"; 
											else if($taxname == "size")
												echo "<span>(One choice only. Make your choice based on the largest measurement. Example: 18\" x 40\" = Large)</span>"; 
										?>
									</h3>
									<div class="taxo_error"></div>
								</div>
								<div class="panel-body panel-content-padding form-group-wrapper <?php echo $taxname; ?>">
									<div class="form-group">
										<div class="col-md-12">
											<?php
												$taxo_html =  wcmp_get_product_terms_HTML( $taxname, $post->ID, apply_filters( 'wcmp_vendor_can_add_'.$taxname, false, get_current_user_id() ) );
												echo $taxo_html;
											?>
										</div>
									</div>
					...
					<?php if ( $product_tags ) : ?>
						<div class="panel panel-default pannel-outer-heading">
							<div class="panel-heading">
								<h3 class="pull-left"><b><?php esc_html_e( 'Artwork Tags (Keywords)', 'woocommerce' ); ?></b></h3>
							</div>
							<div class="panel-body panel-content-padding form-group-wrapper">
								<div class="form-group">
									<div class="col-md-12">
