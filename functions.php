...
/*DISABLED:add_filter( 'wp_nav_menu_items', 'ec_product_categories_nav_menu_items',10,2 );
...
//try to add address2
add_shortcode('vendor_address2','ec_get_vendor_address2');
function ec_get_vendor_address2()
{
	global $post;
	...
	if($location!='')
	{
		return "<p class='ec_vendor_address2'>".$location."</p>";
	}
}
...
function quantity_inputs_for_woocommerce_loop_add_to_cart_link( $html, $product ) {
	...
	if ( $product && $product->is_type( 'simple' ) && $product->is_purchasable() && $product->is_in_stock() && ! $product->is_sold_individually() ) {
    	...
    	//try to add state to WC product archive
    	$html .= '<div class="wcmp_vendor_location"><label>'.do_shortcode("[vendor_address2]").'</label></div>';
        //
...
add_shortcode( 'product_vendor_name', 'ec_display_product_vendor_name' );
function ec_display_product_vendor_name( $atts ){
	global $post;
    $vendor_id =  $post->post_author;
	$fname = get_the_author_meta('first_name',$vendor_id);
	$lname = get_the_author_meta('last_name',$vendor_id);
	...
	$data = '<div class="extra_info statement_info vendor_name">
		<a href="'.$shop_url.'" class="link-red">'.$fname.' '.$lname.'</a>
	</div>';
	}
...
function custom_login(){
 ...
 if( 'wp-login.php' == $pagenow && !is_user_logged_in()) {
  wp_redirect('https://embracecreatives.com/my-account');
  exit();
 }
}*/
...
function set_the_state( $product_id, $product ){
	...
	$v_id = $vendor->id;
	//$v_id = wp_get_current_user()->ID;
	$vendor_state = get_user_meta($v_id, '_vendor_state', true);
	//$username = get_user_by('id', $v_id)->first_name;
